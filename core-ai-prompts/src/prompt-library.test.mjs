...
async function loadShared() {
  const source = await readFile(new URL('./prompt-library.shared.ts', import.meta.url), 'utf8');
  const prompts = await readFile(new URL('./prompts.json', import.meta.url), 'utf8');
...
test('loads a large predefined prompt catalog from bundled JSON', async () => {
  const shared = await readFile(new URL('./prompt-library.shared.ts', import.meta.url), 'utf8');
...
test('all four framework entry points are present', async () => {
  for (const file of ['prompt-library.vue', 'prompt-library.ts', 'prompt-library.react.tsx', 'prompt-library.angular.ts']) {
    assert.ok((await readFile(new URL(file, import.meta.url), 'utf8')).includes('prompt-demo'));
  }
});

test('all framework shells bind the host dark-mode state', async () => {
  for (const file of ['prompt-library.vue', 'prompt-library.ts', 'prompt-library.react.tsx', 'prompt-library.angular.ts']) {
    assert.match(await readFile(new URL(file, import.meta.url), 'utf8'), /is-dark/);
  }
  assert.match(await readFile(new URL('./prompt-library.css', import.meta.url), 'utf8'), /\.prompt-demo\.is-dark/);
});

test('the demo and grid can shrink inside the viewport workspace', async () => {
  const css = await readFile(new URL('./prompt-library.css', import.meta.url), 'utf8');
