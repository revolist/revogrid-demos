...
const css = readFileSync(new URL('../src/hr.css', import.meta.url), 'utf8');
...
test('toolbar selects use a left-aligned value and a shared down-chevron', () => {
  ...
  assert.match(css, /\.hr-select\s*\{[\s\S]*?background-image:\s*url\("data:image\/svg\+xml,[\s\S]*?m1 1 4 4 4-4/);
  assert.match(css, /\.hr-select\s*\{[\s\S]*?background-size:\s*9px 6px;/);
