...
export async function load(parentSelector: string, options: { isDark?: boolean } = {}) {
  ...
  const grid = document.createElement('revo-grid') as HTMLRevoGridElement; grid.className = 'portfolio-grid'; grid.theme = options.isDark ? 'darkCompact' : 'compact'; grid.source = PROJECTS; grid.columns = PROJECT_COLUMNS; grid.grouping = createGrouping(expanded); grid.filter = true; grid.readonly = true; grid.range = true; grid.resize = true; grid.hideAttribution = true; grid.canMoveColumns = true; grid.rowSize = 44; container.appendChild(grid); parent.appendChild(container);
