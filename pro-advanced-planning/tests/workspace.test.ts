...
const structuredSource = readFileSync(new URL('../src/data/planning.structured.ts', import.meta.url), 'utf8')
...
test('uses declarative data-grid formats for every planning value type', () => {
  ...
  assert.match(columnsSource, /prop: 'workflowStatus'[\s\S]*?syncCellTemplate: true,[\s\S]*?cellTemplate: workflowStatusBadgeRenderer/)
...
test('uses high-contrast Status badge defaults with format overrides', () => {
  assert.match(structuredSource, /'not-started':\s*\{\s*label:\s*'Planned',\s*color:\s*'#475569'/)
  assert.match(structuredSource, /'in-progress':\s*\{\s*label:\s*'In progress',\s*color:\s*'#4338ca'/)
  assert.match(structuredSource, /blocked:\s*\{\s*label:\s*'Blocked',\s*color:\s*'#c62828'/)
  assert.match(structuredSource, /done:\s*\{\s*label:\s*'Done',\s*color:\s*'#087443'/)
  assert.match(formattingSource, /colorVariables:\s*\{[\s\S]*?fill:\s*'--badge-cell-appearance-background-color'[\s\S]*?text:\s*'--badge-cell-appearance-color'/)
  assert.match(formattingSource, /class:\s*'badge-cell'/)
  assert.match(formattingSource, /backgroundColor:\s*`var\(--badge-cell-appearance-background-color, \$\{color\}\)`/)
  assert.match(formattingSource, /color:\s*'var\(--badge-cell-appearance-color, #ffffff\)'/)
})
